...
def test_dynamic_enum_constraints(
    sample_payload: dict[str, Any], campo: str, valor: Any
):
    """
...
def test_predict_request_rejects_unknown_enum(sample_payload: dict[str, Any]):
    ...
    assert any(e["loc"] == ("tipo_internet",) for e in errors), (
        "O erro deve apontar para o campo 'tipo_internet'"
    )
...
def test_binary_fields_valid_values(
    sample_payload: dict[str, Any], campo: str, valor: int
):
...
def test_binary_fields_reject_out_of_bounds(
    sample_payload: dict[str, Any], campo: str, valor_invalido: int
):
    ...
    assert "Input should be" in str(excinfo.value) or "less than or equal to" in str(
        excinfo.value
    )
...
def test_predict_response_structure_compliance():
    ...
    """
    data = {"previsao": "Vai cancelar", "probabilidade": 0.85}
