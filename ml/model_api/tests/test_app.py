...
def test_predict_handles_model_crash(client, sample_payload, mocker):
    ...
    mocker.patch.object(wrapper, "predict_single", side_effect=RuntimeError("Falha na Matrix"))
...
def test_predict_handles_generic_exception(client, sample_payload, mocker):
    ...
    mocker.patch.object(wrapper, "predict_single", side_effect=ValueError("Bug Secreto"))
...
def test_batch_failure_behavior(client, sample_payload, mocker):
    ...
    mocker.patch.object(wrapper, "predict_single", side_effect=[sucesso, RuntimeError("Erro Batch 1")])
